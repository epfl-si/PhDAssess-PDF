...
describe('PDF as string generation result', () => {
  it('should generate the PDF', async () => {
    ...
    // 2. ACT
    const pdfGenerated = await makePDFString(sampleData)
    ...
    expect(info.Subject).to.equal("subject of document")
    ...
    expect(JSON.parse(info.Custom.activityLogs)[0]).to.include(
      {referrer: "http://localhost:3000/"}, `Can't find the referere in the activity logs : ${info.Custom.activityLogs}`
    )
